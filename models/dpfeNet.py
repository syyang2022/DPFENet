...
class Decoder(nn.Module):
    def __init__(self, low_level_channels, num_classes):
        ...
        self.spp = DPPM(low_level_channels, 128, 48)
        self.fem = FEM(low_level_channels)
    ...
    def forward(self, x, low_level_features):
        low_level_dppm = low_level_features

        low_level_dppm = self.spp(low_level_dppm)

        low_level_features = self.conv1(self.fem(low_level_features))
        low_level_features = self.relu(self.bn1(low_level_features))

        low_level_features = self.downchannel(torch.cat((low_level_features,low_level_dppm), dim=1))
